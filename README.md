...
4. **Campo** y **resolución**. El campo vacío significa ajustarse al volumen: la imagen cubre todo el plano con píxel igual al menor espaciado, sin recortar ni añadir borde. Al escribir un número de milímetros el campo pasa a ser **cuadrado y centrado**, medido de borde a borde como en un equipo. **Centrar aquí** lo lleva a la referencia de los MPR y **Campo completo** vuelve al ajuste al volumen. La resolución es el tamaño de píxel: *Nativa* usa el menor espaciado del volumen y el resto fija 0,25 a 3 mm. La matriz resultante sale del campo dividido por el píxel y se muestra en el resumen, junto al tamaño que ocupará la exportación.
...
Los cortes se calculan en coordenadas físicas LPS por interpolación trilineal, siempre con píxel cuadrado. Cuando se recorta el campo, las dos líneas naranjas discontinuas marcan sus límites en cada MPR que muestre uno de los dos ejes del plano de salida: con salida coronal, el límite izquierda-derecha se ve en axial y coronal, y el superior-inferior en coronal y sagital. Un píxel menor que el vóxel interpola y se avisa; no añade detalle real. El contenido puede ser el volumen base o la fusión CT + SPECT en color, con la ventana, la paleta, el umbral, la saturación y la opacidad que estén aplicados en ese momento.
...
`node tests.cjs`: 57 pruebas con archivos DICOM sintéticos y volúmenes de referencia. Incluyen lectura, rescale, orden físico, SPECT multiframe, interpolación, identificación y rechazo de geometrías inválidas, PT clásico, BQML, transformación entre coordenadas físicas y texturas 3D, geometría y matriz del reformateo por plano, campo de visión y resolución, combinación del grosor, escritura Secondary Capture comprobada con el lector y el contenedor ZIP.

`node tests-ui.cjs`: 59 pruebas de integración con un DOM/canvas simulado. Verifican los buffers MPR, controles de fusión, referencias, etiquetas PT/Bq/ml, selección de fuente 3D, independencia de escalas, bloqueo por registro, ampliación de los cuatro paneles, zoom por margen, coordenadas de clic después del zoom, planificación de cortes, arrastre del rango, campo y resolución de salida, la pila generada en el cuarto panel, su zoom por margen y la identidad de paciente y estudio de la serie exportada. No sustituyen la comprobación visual en navegador.
