...
- **Botón derecho y arrastrar: navegar por la imagen.** Desplaza la imagen dentro de su panel, que es lo que hace falta cuando el zoom deja parte del corte fuera de la vista. Cada plano se mueve por separado, la referencia y el corte actual no cambian, y el desplazamiento se limita para que nunca quede el panel vacío: siempre permanecen visibles al menos 40 píxeles de imagen. El menú contextual del navegador queda desactivado sobre los lienzos para no interrumpir el arrastre; para guardar la imagen está el botón **PNG**.
- La pila de **CORTES** generados se navega igual: rueda para cambiar de corte, margen para el zoom y botón derecho para desplazar.
- **Restablecer vistas** vuelve al 100 %, al centro de los cortes y deja las cuatro imágenes centradas. Cargar otro volumen base también restablece zoom y desplazamiento.
...
`node tests-ui.cjs`: 68 pruebas de integración con un DOM/canvas simulado. Verifican los buffers MPR, controles de fusión, referencias, etiquetas PT/Bq/ml, selección de fuente 3D, independencia de escalas, bloqueo por registro, ampliación de los cuatro paneles, zoom por margen, coordenadas de clic después del zoom y después de desplazar con el botón derecho, el límite del desplazamiento, los avisos al abrir sin seleccionar o con un solo corte, planificación de cortes, arrastre del rango, FoV y matriz de salida, el nombre editable de la serie, la exportación PNG del cuarto panel, la pila generada, su zoom por margen y la identidad de paciente y estudio de la serie exportada. Añaden 5 pruebas de exportación que recorren el guardado completo con un sustituto del selector de carpetas: la carpeta propia de cada serie, el nombre de cada archivo, el recuento de la ventana de guardado, la cancelación y el fallo de escritura. No sustituyen la comprobación visual en navegador.
